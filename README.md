...
- `-t` フラグを付けると、フロントマターの `tags` を抽出して一覧表示します。単一ファイルを渡した場合はそのファイル内のタグを、ディレクトリを渡した場合は配下の Markdown を再帰的に探索して集約したタグを表示し、選択したタグを含むすべてのファイルパスを出力します。キャンセルするとそのまま終了します。
